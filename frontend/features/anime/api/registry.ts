...
export const SCORE_NOTES_REGISTRY: Record<AnimeTrackerNote, {
  ...
  [AnimeTrackerNote.USED_SCORE_AVG_ANIME]: {
    ...
    note$: null,
    description$: "Score is calculated by taking the average of all seasons"
  },
  ...
  [AnimeTrackerNote.INCLUDED_SPECIAL_EPS_SCORE]: {
    name$: "Score included special episodes",
  ...
  [AnimeTrackerNote.NOT_INCLUDED_SPECIAL_EPS_SCORE]: {
    name$: "Score did not include special episodes",
  ...
  [AnimeTrackerNote.MANGADEX_SCORE_ONLY]: {
    ...
    description$: "...since there's no rating on MAL, and Mangadex ratings tend to be inflated.",
  ...
  [AnimeTrackerNote.NOT_LIGHT_NOVEL]: {
    ...
    description$: "Reality... this is from a novel, but it have been grouped under '(L)N' for simplicity.",
  ...
  [AnimeTrackerNote.FROM_YT]: {
    ...
    description$: "Note that there's a small chance that some episode(s) are not available, because youtube can be youtube-y sometime",
  ...
  [AnimeTrackerNote.LOCAL_ARCHIVE]: {
    name$: "A local copy of this has been saved",
  ...
  [AnimeTrackerNote.MANGADEX_MISSING_CHAPTERS]: {
    ...
    description$: "Some of the chapters is not translated into english, or all chapters are being listed as \"This chapter is not available\"",
    note$: null
  },
  [AnimeTrackerNote.ONE_SHOT]: {
    name$: "This is a one-shot",
    description$: "(.statement) - a one chapter story only, and usually a short story.",
    note$: null
  }
}
