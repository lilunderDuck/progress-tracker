...
func FormatPath(somePath string) string {
	if somePath == "" {
		return FormatWith(COLOR_GRAY, "", "[... empty path ...]")
	}
...
func FormatNumber[T AnyNumber](number T) string {
	return FormatWith(COLOR_GREEN, STYLE_NONE, strconv.Itoa(int(number)))
}

type AnyFloatNumber interface {
	float32 | float64
}

func FormatFloatNumber[T AnyFloatNumber](number T) string {
	return FormatWith(COLOR_GREEN, STYLE_NONE, fmt.Sprintf("%f", number))
}
