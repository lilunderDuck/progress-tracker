...
var internalDbPool = map[string]*GDStore{}

// New creates a new GDStore
func New(filePath string) *GDStore {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "opening %s", debug.FormatPath(filePath))
	}
	...
	internalDbPool[filePath] = store
	return store
}

func GetInstanceOrElseOpen(filePath string) *GDStore {
	dbInstance, ok := internalDbPool[filePath]
	if !ok {
		if flags.DEBUG_MODE {
			debug.WarnLabelf("gdstore", "database is not opened: %s, attempting to reopen...", debug.FormatPath(filePath))
		}
		dbInstance = New(filePath)
	}

	return dbInstance
}

func GetInstance(filePath string) *GDStore {
	return internalDbPool[filePath]
}

func CloseAll() {
	for _, activeDbInstance := range internalDbPool {
		delete(internalDbPool, activeDbInstance.FilePath)
	}
}
...
func (store *GDStore) Get(key string) (value []byte, ok bool) {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "get value with key: %s", key)
	}
...
func (store *GDStore) GetString(key string) (valueAsString string, ok bool) {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "get string with key: %s", key)
	}
...
func (store *GDStore) GetInt(key string) (valueAsInt int, ok bool, err error) {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "get int with key: %s", key)
	}
...
func (store *GDStore) Put(key string, value []byte) error {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "set: %s = %.20s", key, value)
	}
...
func (store *GDStore) PutAll(entries map[string][]byte) error {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "putting all values...")
	}
...
func (store *GDStore) Delete(key string) error {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "delete value with key: %s", key)
	}
...
func (store *GDStore) GetPage(page, pageSize int) PageResult[map[string]string] {
	store.mux.RLock()
	defer store.mux.RUnlock()

	total := store.data.Len()
	if page < 1 {
		page = 1
	}
	if pageSize < 1 {
...
// GetPageKeys fetches a paginated slice of keys only
func (store *GDStore) GetPageKeys(page, pageSize int) PageResult[string] {
	store.mux.RLock()
	defer store.mux.RUnlock()
	...
	pageKeys := make([]string, end-start)
	copy(pageKeys, keys[start:end])
...
func (store *GDStore) ValuesAsStringFast() string {
	if flags.DEBUG_MODE {
		debug.InfoLabelf("gdstore", "fast getting all items...")
	}

	first := true
	var builder strings.Builder
	builder.WriteString("[")
	// db.Values() return [][]byte, so we can't just slap a json.Unmarshal() here...
	for _, rawEntry := range store.Values() {
		if !first {
			builder.WriteString(",")
		}
		builder.WriteString(string(rawEntry))
		first = false
	}
	builder.WriteString("]")
